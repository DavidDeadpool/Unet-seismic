...
class _Transition(nn.Module):
    def __init__(self, num_input_features, num_output_features,mode='pool'):
        ...
        self.add_module('conv', nn.Conv2d(num_input_features, num_output_features,
                                          kernel_size=1, stride=1, bias=False))
        if mode=='pool':
            self.add_module('resize', nn.AvgPool2d(kernel_size=2, stride=2))
        else:
            self.add_module('resize', nn.UpsamplingBilinear2d(scale_factor=2))
        self._init_weight()
...
class DenseUNet(nn.Module):
    def __init__(self,num_block_layer, block_out_channls, growth_rate, efficient, drop_out=0, use_context=False, use_aspp=False, gc_fusion_method=['channel_mul']):
        ...
        self.add_module("conv1",nn.Conv2d(in_channels=3,out_channels=block_out_channls,kernel_size=5,padding=2))
        ...
        self.add_module("classifier",nn.Conv2d(num_block_layer * growth_rate + block_out_channls, 2, kernel_size=1, stride=1))
...
if __name__ == '__main__':
    ...
    Net = DenseUNet(block_out_channls=32, num_block_layer=4, efficient=True, use_context=True,growth_rate=16).cuda()
    # for i in range(1):
    #     out = Net(x)
    #     sum = out.sum()
    #     sum.backward()
